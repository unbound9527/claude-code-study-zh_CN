...
**作者使用MiniMax的 49元/月 Token Plan 套餐**，整体能力逊色于Claude，但按调用次数计算，足够个人日常开发使用，复杂项目可作为辅助。通过作者分享的链接购买，可以享[9折](https://platform.minimaxi.com/subscribe/token-plan?code=Gnf6KDMG1h&source=link)。

> ⚠️ **注意**：切换到 MiniMax 等国产大模型后，**无法使用 Claude Code 官方的 WebSearch 功能**。如需 WebSearch，可配置 MiniMax 的 MCP 服务，详见 [MiniMax MCP 配置指南](https://platform.minimaxi.com/docs/guides/token-plan-mcp-guide#%E6%89%8B%E5%8A%A8%E9%85%8D%E7%BD%AE)。
...
MiniMax 提供 WebSearch 功能（通过 MCP），配置方式如下：

**方式一：MCP 方式（推荐，可使用 web_search）**
```bash
# 添加 MiniMax MCP
claude mcp add -s user MiniMax \
  --env MINIMAX_API_KEY=你的APIKey \
...
> ⚠️ **WebSearch 功能说明**：MiniMax 的 `web_search` 工具通过 MCP 提供，非官方 WebSearch。配置 MCP 后可用 `/mcp` 查看工具列表。

**MiniMax 订阅与 MCP 配置详见**：[MiniMax MCP 配置指南](https://platform.minimaxi.com/docs/guides/token-plan-mcp-guide#%E6%89%8B%E5%8A%A8%E9%85%8D%E7%BD%AE)
